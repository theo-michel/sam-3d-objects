...
from services.segmentation_moondream import segment_image_with_moondream
...
def decode_rle_string(rle_string: str, height: int, width: int) -> np.ndarray:
    ...
    try:
        ...
        if HAS_PYCOCOTOOLS:
            rle_obj = {
                'counts': rle_string if isinstance(rle_string, bytes) else rle_string.encode('utf-8'),
                'size': [height, width]
            }
...
def visualize_results(image_path: str, results: list[dict], save_dir: str = "output_moondream"):
    """
    Visualize the segmentation results from Moondream + SAM3.
    """
    ...
    for i, result in enumerate(results):
        ...
        if rle_string:
            ...
            mask_pil.save(os.path.join(save_dir, f"mask_obj_{i}.png"))
            ...
            ax.imshow(original_image)
            ax.text(0.5, 0.5, "No Mask", ha='center', va='center', color='red')
        ...
        if box is not None and len(box) == 4:
            ...
            rect = patches.Rectangle(
                (x, y), width, height,
                linewidth=2, edgecolor='lime', facecolor='none', label='SAM3 Box'
            )
            ax.add_patch(rect)
            
        # Draw Moondream Source Box (Blue)
        if source_object:
            ...
            rect_md = patches.Rectangle(
                (mx_min, my_min), mw, mh,
                linewidth=2, edgecolor='cyan', facecolor='none', linestyle='--', label='Moondream Box'
            )
    ...
    plt.tight_layout()
    output_path = os.path.join(save_dir, "moondream_results.png")
...
def test_local_image(image_path: str, prompt: str):
    ...
    print(f"Testing Moondream segmentation with prompt: '{prompt}'")
    try:
        results = segment_image_with_moondream(image_url, prompt=prompt)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Moondream segmentation with a local image.")
    parser.add_argument("image_path", type=str, help="Path to the local image file")
    parser.add_argument("--prompt", type=str, default="Points of interest", help="Prompt for Moondream")
