...
def visualize_results(image_path: str, results: list[dict], save_dir: str = "output_yolo"):
    """
    ...
    for i, result in enumerate(results):
        ...
        if rle_string:
            ...
            ax.imshow(original_image)
            ax.text(0.5, 0.5, "No Mask", ha='center', va='center', color='red', transform=ax.transAxes)
        ...
        if source_object:
            ...
            rect_yolo = patches.Rectangle(
                (mx_min, my_min), mw, mh,
                linewidth=2, edgecolor='cyan', facecolor='none', linestyle='--', label='YOLO Box'
            )
...
def test_local_image(image_path: str, prompt: str):
    ...
    try:
        results = segment_image_with_yolo(image_url, prompt=prompt, local_image_path=image_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test YOLOv12 + SAM3 segmentation with a local image.")
    parser.add_argument("image_path", type=str, help="Path to the local image file")
    parser.add_argument("--prompt", type=str, default="Pickable objects", help="Prompt (ignored by standard YOLO)")
