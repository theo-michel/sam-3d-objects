...
from services.segmentation import upload_image_to_fal, segment_image
...
def visualize_masks(image_path: str, segmentation_result: dict, save_dir: str = "output"):
    """
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        test_images = list(Path(".").glob("test*.jpg")) + list(Path(".").glob("test*.png")) + list(Path(".").glob("test*.jpeg"))
        if test_images:
            image_path = str(test_images[0])
        else:
            print("Usage: python test_segmentation.py <image_path>")
            print("Or place a test image (test.jpg, test.png, etc.) in the current directory")
            sys.exit(1)
